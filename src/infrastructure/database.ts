...
export class Database implements IDatabase {
    ...
    private isTransientConnectionError(err: Error): boolean {
    ...
    async query<T extends QueryResultRow = QueryResultRow>(sql: string, params: SqlParam[] = []): Promise<T[]> {
        ...
        for (let attempt = 0; attempt <= QUERY_RETRY_DELAYS_MS.length; attempt++) {
            try {
                ...
                const hasNextAttempt = attempt < QUERY_RETRY_DELAYS_MS.length;
                if (hasNextAttempt && this.isTransientConnectionError(error)) {
                    const delay = QUERY_RETRY_DELAYS_MS[attempt];
                    this.logger.warn(
                        `Transient SQL connection error, retrying in ${delay}ms (attempt ${attempt + 1}/${QUERY_RETRY_DELAYS_MS.length + 1}): ${error.message}`,
                    );
                ...
                this.logger.error(`SQL Error:\n${sql}\n${this.formatQueryErrorDetails(error, params)}`);
                throw err;
            }
        }
