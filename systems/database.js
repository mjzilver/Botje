class Database {
	...
	initializeDatabase() {
        ...
        this.db.run(`CREATE TABLE IF NOT EXISTS messages (user_id TEXT, user_name TEXT, message TEXT, date TEXT, channel TEXT, server TEXT, reactions INTEGER, PRIMARY KEY(user_id, date, channel))`)
	...
	storemessage(message) {
		...
		&& !message.author.bot && !message.content.match(new RegExp(config.prefix, "i")) && !message.content.match(new RegExp("^t!", "i"))) {
			var insert = this.db.prepare('INSERT OR IGNORE INTO messages (user_id, user_name, message, channel, server, reactions, date) VALUES (?, ?, ?, ?, ?, ?, ?)',
				[message.author.id, message.author.username, message.cleanContent, message.channel.id, message.guild.id, message.reactions.cache.size, message.createdAt.getTime()]);
