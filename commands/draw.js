...
module.exports = function draw(message) {
    ...
    image.fillRect(0, 0, totalImageSize, totalImageSize, { red: 255, green: 255, blue: 255,  alpha: 255 })
    ...
    db.all(selectSQL, [], async (err, rows) => {
        ...
        for (var i = 0; i < rows.length; i++)
            if (rows[i].x >= 0 && rows[i].x < config.image.size && rows[i].y >= 0 && rows[i].y < config.image.size)
                image.fillRect(rows[i].x * config.image.magnification, rows[i].y * config.image.magnification, config.image.magnification, config.image.magnification, { red: rows[i].red, green: rows[i].green, blue: rows[i].blue, alpha: 255 })
