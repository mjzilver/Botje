...
module.exports = {
    ...
    "function": async function weather(message) {
        ...
        try {
            const response = await axios.get(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${config.weather_api_key}&units=metric&mode=JSON`)
            ...
            logger.error(err)
            bot.messageHandler.send(message, "Failed to fetch weather data")
        }
    }
}